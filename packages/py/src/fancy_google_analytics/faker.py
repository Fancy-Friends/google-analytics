...
def _report_run(config: dict[str, Any], fake: FakeValues) -> Any:
    return {
        "dimensionHeaders": [
            {
                "name": "date",
...
def respond(operation: str, request: dict[str, Any]) -> Any:
    ...
    if operation == "report_run":
        return _report_run(config, fake)
    ...
    raise ValueError(
        f'google_analytics: no fake response is defined for "{operation}". '
