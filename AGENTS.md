...
npm run provider -- google-analytics            # regenerate this repo
npm run provider -- google-analytics --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/google-analytics-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/google-analytics-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/google-analytics-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-google-analytics` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/google-analytics/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/google-analytics` |
| npm | `@particle-academy/google-analytics-ui`, `@particle-academy/google-analytics-js` |
| Packagist | `particle-academy/google-analytics-php` |
| PyPI | `fancy-google-analytics` |
...
**Default to plain HTTP.** A Google Analytics SDK is third-party code and is subject to
